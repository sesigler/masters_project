...
exp_fun = read.table(paste0(mac_dir, 'MAC_bin_estimates_', Pop, '_fun_100.txt'), header=T, sep='\t')
exp_fun_conf = read.table(paste0(mac_dir, 'MAC_bin_estimates_', Pop, '_fun_', p_conf, '.txt'), header=T, sep='\t')
exp_syn_conf = read.table(paste0(mac_dir, 'MAC_bin_estimates_', Pop, '_syn_', p_conf, '.txt'), header=T, sep='\t')
...
for(j in 1:100){
  ...
  # subset the datasets for 80% pruning
  hap_cc_pruned = hap_cc_conf[, cc]
  hap_case_pruned80 = hap_cc_conf[, cases]
  ...
  fwrite(hap_case_pruned80, paste0(dir_out, 'chr19.block37.', Pop, '.sim', j, '.cases.', p_conf, 'fun.', p_conf, 'syn.haps.gz'),
         quote=F, row.names=F, col.names=F, sep=' ')
