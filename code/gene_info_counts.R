...
# dir = paste0('/home/math/siglersa/admixed/AFR_NFE_pops/160v100v80/') # admixed 160v100v80
dir_out = paste0('/home/math/siglersa/mastersProject/Output/gene_info/')
# dir_out = paste0('/home/math/siglersa/mastersProject/Output/gene_info/23K_', admixed, '/')
...
for (i in 1:100){
  ...
  hap_gene = hap2 %>% filter(sums != 0, exonic != 'intronic') %>% group_by(gene, fun) %>% count()
  counts_wide = tidyr::pivot_wider(hap_gene, names_from = fun, values_from = n, values_fill = 0) %>% mutate(ratio = fun/syn)
  
  counts = rbind(counts, counts_wide)
  
  print(i)
}

write.table(as.data.frame(counts), paste0(dir_out, 'fun_syn_counts_', Nsim, '_', Pop, '_', folder, '.txt'), quote=F, row.names=F, col.names=T)
