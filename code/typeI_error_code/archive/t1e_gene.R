...
# data = 'by_gene'
p_case = 160
p_case_fun = p_exp = p_case_syn = p_int_fun = p_int_syn = int_prune = 100
...
dir_leg = paste0('/home/math/siglersa/mastersProject/20K_NFE/', pruning, '/', folder, '/') #nfe
dir_in = paste0('/home/math/siglersa/mastersProject/20K_NFE/', pruning, '/', folder, '/') #nfe
# dir_out = paste0('/home/math/siglersa/mastersProject/Output/', pruning, '/', data, '/', folder, '/')
# dir_leg = paste0('/home/math/siglersa/mastersProject/20K_', Pop, '/', folder, '/') #afr
# dir_in = paste0('/home/math/siglersa/mastersProject/20K_', Pop, '/', folder, '/') #afr
...
# dir_leg = paste0('C:/Users/sagee/Documents/HendricksLab/mastersProject/input/20K_', Pop, '/', folder, '/')
# dir_in = paste0('C:/Users/sagee/Documents/HendricksLab/mastersProject/input/20K_', Pop, '/', folder, '/')
...
for (i in 1:10){
   ...
   #############################################################################
   # Save cc AC and AF info to plot
   cc_leg = count_cc %>% mutate(id = leg$id, gene = leg$gene, fun = leg$fun)
   common_ext = leg[which(count_cases$maf > maf | count_cc$maf > maf),]
   common_id = common_ext$id
   cc_leg = cc_leg %>% mutate(common = ifelse(id %in% common_id, "common", "rare"))
   cc_data = rbind(cc_data, cc_leg)
...
results = data.frame(cc_data)
write.table(results, paste0(dir_out, "20K_", Pop, "_cc_mac_maf_info_", int_prune, "_v_", ext_prune, "_maf", maf, ".txt"), quote=F, row.names=F, col.names=T)
