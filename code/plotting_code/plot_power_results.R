...
calc = 'Power'
Pops = c('AFR', 'NFE') # c('IAM', 'NFE', 'EAS', 'AFR')
admx_props = c(80, 20) # c(47, 44, 5, 4)
scen = 's2'
int_admx = '100% AFR' # '75% IAM, 19% NFE, 3% EAS, 3% AFR'
ext_admx = '80% AFR, 20% NFE' # '47% IAM, 44% NFE, 5% EAS, 4% AFR' 
...
str_Nrefs = 'Nref AFR: 5000, Nref NFE: 5000' # 'Nref IAM: 5000, Nref NFE: 5000, Nref EAS: 5000, Nref AFR: 5000'
...
results2$Nint5K = factor(results2$Nint5K, levels=c("default", "Ncase1K_Nic4K_Nref5K", "Ncase2K_Nic3K_Nref5K", "Ncase3K_Nic2K_Nref5K", "Ncase4K_Nic1K_Nref5K"),
                         labels = c("Cases: 2000, Internal Controls: 2000", "Cases: 1000, Internal Controls: 4000", "Cases: 2000, Internal Controls: 3000",
                                    "Cases: 3000, Internal Controls: 2000", "Cases: 4000, Internal Controls: 1000"))
...
results2$Nint5K = factor(results2$Nint5K, levels=c("default", "Ncase1K_Nic4K_Nref5K", "Ncase2K_Nic3K_Nref5K", "Ncase3K_Nic2K_Nref5K", "Ncase4K_Nic1K_Nref5K"),
                         labels = c("(2K, 2K)", "(1K, 4K)", "(2K, 3K)", "(3K, 2K)", "(4K, 1K)"))
...
colors3 = c("#0072B2", "#E69F00", "#D81B60", "#999999", "#BC9F4C", "#009E73")
...
# power for new admixed AFR results-Total Internal Sample Size
pNint5K <- ggplot(results2 %>% filter(!(Method == "LogProx (External)" | Method == "LogProx (Internal + External)" | 
                                          (Method == "ProxECAT (External)" & MACs == "Unadjusted") | (Method == "ProxECAT-weighted (External)" & MACs == "Unadjusted") |
                                          (Method == "iECAT-O (Internal + External)" & MACs == "Unadjusted") |
                                          MACs == "Adjusted Ncc" | Nint5K == "Cases: 2000, Internal Controls: 2000") & 
                                        (Gene == "ADGRE5" | Gene == "ADGRE3" | Gene == "TECR")), 
  ...
  # scale_linetype_manual(values = c("dashed", "solid")) +
  # scale_size_manual(values = c(0.5, 0.8)) +
  facet_wrap(~Nint5K, ncol = 2, scales = 'fixed') +
  # facet_wrap(~Data, ncol = 1) +
  labs(y='Power', x='Gene', title=paste0('Power by Gene and Total Internal Sample Size \nCases: 160% pruned, ', int_admx, '\nInternal Controls: ', int_prune, '% pruned, ', int_admx,
                                         '\nCommon Controls: ', ext_prune, '% pruned, ', ext_admx, '\nNcc: ', Ncc, ', ', str_Nrefs, ', MAF: ', maf)) +
  # '\nPop: Admixed ', admx, " ", Pop1, '-', Pop2, ', Nsim: ', Nsim, ', Ncase: ', Ncase, ', Ncc: ', Ncc, ', Nref: ', Nref, ', MAF: 0.001')) +
  # theme(axis.text.x = element_text(angle = 35, hjust=0.65))
  theme_bw(base_size = 14)
pNint5K
ggsave(file = paste0(dir_out, calc, '_v1_gene_', file_out), plot = pNint5K, height = 8, width = 16, units = 'in')
...
pNint5K <- ggplot(results2 %>% filter(!(Method == "LogProx (External)" | Method == "LogProx (Internal + External)" | 
                                          (Method == "ProxECAT (External)" & MACs == "Unadjusted") | (Method == "ProxECAT-weighted (External)" & MACs == "Unadjusted") |
                                          (Method == "iECAT-O (Internal + External)" & MACs == "Unadjusted") |
                                          MACs == "Adjusted Ncc" | Nint5K == "(2K, 2K)") & 
                                        (Gene == "ADGRE5" | Gene == "ADGRE3" | Gene == "TECR")), 
                aes(x=Nint5K, y=Value, color=Method)) +
  geom_point(size=1.8, position=position_dodge(width=0.8)) +
  geom_hline(yintercept=0.80, linetype=2, linewidth=0.5) +
  # geom_hline(yintercept=1, linetype="blank", linewidth=1.5) +
  # scale_y_continuous(limits=c(0, 1)) +
  ...
  facet_wrap(~Gene, ncol = 1, scales = 'fixed') +
  # facet_wrap(~Data, ncol = 1) +
  labs(y='Power', x='Internal Sample Size (Cases, Internal Controls)', title=paste0('Power by Gene and Total Internal Sample Size \nCases: 160% pruned, ', int_admx, '\nInternal Controls: ', int_prune, '% pruned, ', int_admx,
                                                         '\nCommon Controls: ', ext_prune, '% pruned, ', ext_admx, '\nNcc: ', Ncc, ', ', str_Nrefs, ', MAF: ', maf)) +
  # '\nPop: Admixed ', admx, " ", Pop1, '-', Pop2, ', Nsim: ', Nsim, ', Ncase: ', Ncase, ', Ncc: ', Ncc, ', Nref: ', Nref, ', MAF: 0.001')) +
  # theme(axis.text.x = element_text(angle = 35, hjust=0.65))
  theme_bw(base_size = 14)
pNint5K
ggsave(file = paste0(dir_out, calc, '_v2_gene_', file_out), plot = pNint5K, height = 8, width = 16, units = 'in')
